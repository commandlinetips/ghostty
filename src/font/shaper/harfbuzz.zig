...
pub const Shaper = struct {
    ...
    pub fn shape(self: *Shaper, run: font.shape.TextRun) ![]const font.shape.Cell {
        ...
        //
        // Note: std.mem.reverse reverses the entire Cell struct including cluster values,
        // which correctly maintains the glyph-to-cluster mapping after reversal.
        const is_rtl = self.hb_buf.getDirection() == .rtl;
        if (is_rtl) {
            log.debug("RTL text detected, reversing glyphs for terminal grid", .{});
    ...
    pub const RunIteratorHook = struct {
        ...
        pub fn prepare(self: *RunIteratorHook) !void {
            ...
            // NOTE: RTL support is being added. The text direction will be
            // determined by HarfBuzz's guessSegmentProperties() call in finalize(),
            // which analyzes the actual text content. By not forcing LTR here,
            // we allow HarfBuzz to detect the proper direction based on the text.
            //
            // Previously we forced LTR because the terminal grid model assumes
            // strictly increasing X values. This is being addressed in Phase 3
            // with cluster mapping support.
            //
            // Set initial direction to LTR (default). For RTL scripts (Arabic, Hebrew),
            // the direction will be explicitly set to RTL later (see addCodepoint) before
            // finalize() calls guessSegmentProperties(), which may still adjust other
            // segment properties.
